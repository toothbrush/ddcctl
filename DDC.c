...
//  based on: https://github.com/glfw/glfw/pull/192/files
{
    ...
    // backreference the given displayID to its CGDisplay[#] ...
    // ...which I hope corresponds with its IOKit "Unit Number"
    // https://github.com/glfw/glfw/blob/3.2.1/src/cocoa_monitor.m#L244 _glfwPlatformGetMonitors
    uint32_t i, displayCount;
    ...
    int displayIndex; // = ??? displays.indexOf(displayID)
    ...
    while ((serv = IOIteratorNext(iter)) != MACH_PORT_NULL)
    {
        ...
        */
            /*
             Apple IOKit documentation states unitnumber follows NSSpace>framebuffer>GPU and not actual monitors:
             ...
             * For a particular hardware configuration, this value will not change when the attached monitor is changed.
             * The number will change, though, if the I/O Kit device tree changes, for example,
             *     when hardware is reconfigured, drivers are replaced, or significant changes occur to I/O Kit.
             * Therefore keep in mind that this number may vary across login sessions.
             */
        if (CGDisplayUnitNumber(displayID) != displayIndex ) // unitnumber seems to correspond with the NSScreen index of the CGDisplay...
        {
            CFRelease(info);
            continue;
        }
